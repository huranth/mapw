...
import { CheckIcon, CliIcon, CopyIcon, TerminalIcon, XIcon } from "@/components/Icons";
...
export const TerminalNode = memo(function TerminalNode({ id, data }: NodeProps) {
    ...
    <div className="node-terminal-wrap" data-testid={`node-${id}`}>
      <div className="node-terminal">
        ...
        <header className="node-terminal__header">
          ...
          <span className="node-terminal__focus-tag" aria-hidden="true" data-testid={`node-${id}-focus-tag`}>Focus</span>
          <div className="node-terminal__chips nodrag" data-testid={`node-${id}-chips`} hidden={tuiRunning}>
            {cliTools.map((cli) => (
              <button key={cli.id} type="button" className="node-terminal__chip nodrag" aria-label={`Launch ${cli.name} in ${id}`} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); void window.bridge.ptyWrite(id, `${cli.launchCommand}\r`).catch(() => {}); }}><CliIcon id={cli.id} size={11} style={{ marginRight: 5 }} />{cli.name}</button>
            ))}
          </div>
          <button type="button" className={`node-terminal__copy nodrag${copied ? " node-terminal__copy--copied" : ""}`} aria-label={`Copy ${id} content`} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); onCopyClick(); }}>{copied ? <><CheckIcon size={11} style={{ marginRight: 4, color: "#059669" }} />Copied</> : <>Copy<CopyIcon size={11} style={{ marginLeft: 4, color: "#2563EB" }} /></>}</button>
          <button type="button" className="node-terminal__close nodrag" aria-label={`Close ${id}`} onMouseDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); removeNode(id); }}><XIcon size={12} style={{ color: "#DC2626" }} /></button>
        </header>
