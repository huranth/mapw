...
async function fetchLatestRelease(): Promise<LatestRelease | null> {
  ...
  return parseLatestJson(await res.text());
}
...
export async function runUpdateCycle(ctx: UpdateContext): Promise<void> {
  try {
    ...
    const workDir = join(tmpdir(), `mapw-update-${Date.now()}`);
    ...
    const applyOnQuit = (): void => {
      ...
      void writeFile(scriptPath, buildSwapScript(exePath, process.execPath), "utf8").then(() => {
        const child = spawn(
          "powershell.exe",
