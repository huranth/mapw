...
function bashRc(profilePath: string | null): string {
  ...
  return [
    ...
    "",
    "__bs_in_command=0",
    "__bs_debug(){ local rc=$?; if [ \"$__bs_in_command\" = \"0\" ]; then __bs_in_command=1; printf '\\033]133;C\\007'; fi; return $rc; }",
    "trap '__bs_debug' DEBUG",
    "",
    "__bs_urlenc(){ printf '%s' \"$1\" | sed -e 's/%/%25/g' -e 's/ /%20/g' -e 's/#/%23/g' -e 's/?/%3F/g' -e 's/;/%3B/g' -e 's/&/%26/g'; }",
    "__bs_precmd(){ local ec=$?; if [ \"$__bs_in_command\" = \"1\" ]; then printf \"\\033]133;D;%s\\007\" \"$ec\"; __bs_in_command=0; fi; local h; h=\"$(uname -n 2>/dev/null || hostname 2>/dev/null || echo localhost)\"; printf \"\\033]7;file://%s%s\\007\" \"$h\" \"$(__bs_urlenc \"$PWD\")\"; printf \"\\033]133;A\\007\"; }",
    "if [ -z \"$PROMPT_COMMAND\" ]; then PROMPT_COMMAND='__bs_precmd'; else PROMPT_COMMAND=\"__bs_precmd; $PROMPT_COMMAND\"; fi",
    "",
  ].join("\n");
}
...
function zshRc(profilePath: string | null): string {
  ...
  return [
    ...
    ...(src ? [src] : []),
    "",
    "__bs_preexec(){ printf '\\033]133;C\\007'; }",
    "__bs_urlenc(){ printf '%s' \"$1\" | sed -e 's/%/%25/g' -e 's/ /%20/g' -e 's/#/%23/g' -e 's/?/%3F/g' -e 's/;/%3B/g' -e 's/&/%26/g'; }",
    "__bs_precmd(){ local ec=$?; local h; h=\"$(uname -n 2>/dev/null || hostname 2>/dev/null || echo localhost)\"; printf \"\\033]133;D;%s\\007\" \"$ec\"; printf \"\\033]7;file://%s%s\\007\" \"$h\" \"$(__bs_urlenc \"$PWD\")\"; printf \"\\033]133;A\\007\"; }",
